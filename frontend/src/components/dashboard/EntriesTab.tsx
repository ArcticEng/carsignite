...
export function EntriesTab() {
  ...
  if (!data) return <div className="glass-card p-7 text-center text-[#6E7275]">Could not load entries</div>;
    ...
    <div className="glass-card p-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-5">
        {[
          ...
          { label: 'Wins', value: data.totalWins, color: data.totalWins > 0 ? 'text-ci-green' : 'text-[#6E7275]' },
        ].map(s => (
          <div key={s.label} className="glass-card p-4 relative overflow-hidden">
            ...
            <div className="text-[10px] font-semibold text-[#6E7275] tracking-[2px] uppercase mb-1">{s.label}</div>
      ...
      <div className="glass-sm p-4 mb-4">
        ...
        {pr.prize_desc && <div className="text-[13px] text-[#6E7275] mt-1">{pr.prize_desc}</div>}
      ...
      <div className="bg-[rgba(34,204,110,.06)] border border-[rgba(34,204,110,.15)] rounded-xl p-4 text-center">
        ...
        <div className="text-xs text-[#6E7275] mt-1">{data.entriesPerCycle}× weighted entries</div>
      </div>
    </div>
  );
}
