...
export function MapTab() {
  ...
  if (groups.length === 0) return (
    <div className="glass-card p-7 text-center py-14">
      ...
      <div className="font-semibold text-[#9898a8]">No Groups Yet</div>
      <p className="text-[13px] text-[#58586a] mt-1">Join or create a group (Drives tab). Location sharing is group-only.</p>
    </div>
  );
  ...
  if (!activeGroup) return (
    <div className="glass-card p-5">
      ...
      <p className="text-[13px] text-[#58586a] mb-4">Select a group to track members. Only your group members can see you.</p>
      {groups.map(g => (
        <button key={g.id} onClick={() => setActiveGroup(g.id)}
          className="w-full glass-sm flex items-center justify-between p-3.5 mb-2 text-left hover:border-[rgba(230,57,70,.2)] transition-all">
          <div className="flex items-center gap-2.5">
            <span className="text-[22px]">{g.emoji || '🏎️'}</span>
            <div><div className="font-bold text-sm">{g.name}</div><div className="text-[11px] text-[#58586a]">{g.member_count} members</div></div>
          </div>
  ...
  return (
    <div className="glass-card p-5">
      ...
      <div className="w-full rounded-xl border border-[rgba(230,57,70,.08)] bg-bg-2 flex items-center justify-center" style={{ height: 'min(420px, 55vh)' }}>
        <div className="text-center">
          <span className="text-4xl block mb-2">🗺️</span>
          <span className="text-[#58586a] text-sm">Set GOOGLE_MAPS_API_KEY in .env</span>
        </div>
      </div>

      <div className="mt-3">
        {positions.length === 0 ? (
          <div className="text-center py-3 text-xs text-[#58586a]">No members sharing. Click &quot;Share Location&quot; to start!</div>
        ) : positions.map(p => {
          ...
          return (
            <div key={p.id} className="glass-sm flex items-center justify-between px-3 py-2 mb-1">
              ...
              <div className="text-[10px] text-[#58586a]">{ago}m ago{p.speed > 0 ? ` · ${Math.round(p.speed)}km/h` : ''}</div>
            </div>
          );
        })}
      </div>
